...
面向中国 A 股与国内期货的本地优先、AI 主导量化研究与交易平台。
...
AstraQuant 不是在传统量化软件末端附加一个聊天机器人，而是让 AI 贯穿研究、特征发现、模型训练、信号生成、市场状态识别和复盘，同时由确定性系统负责数据一致性、回测核算、风险控制与订单执行：

```text
离线循环：历史数据 → 特征工程 → AI 训练 → 回测验证 → 模型发布
在线循环：实时行情 → 在线特征 → AI 推理 → 结构化信号
          → 确定性风控 → OMS → 模拟/实盘执行 → 监控与复盘
```
...
- **确定性安全边界**：AI 生成信号或目标仓位，事件驱动核心负责回测、风控、订单、对账和执行。
...
5. 将在线推理接入 Paper Trading、订单状态、基础风控和交易复盘。
6. 通过适配层接入国内期货 CTP 和 A 股券商通道，并保留人工安全门。
