...
> 当前状态：Phase 3A `IN_PROGRESS`。桌面端已建立东财掘金 Python SDK 隔离桥、真实行情
> 契约、六个核心指数、持久化自选/搜索/分时、订阅配额、断线状态和 Windows 凭据存储；正式
> 首页不再使用模拟行情或硬编码点位。尚未完成真实 A 股交易时段的 30 分钟验收，也尚未
> 接入 AI 策略、Paper 模拟或实时买卖点。项目不连接真实交易账户。
...
- **人工交易边界**：AI 生成买卖点、仓位和风险建议，真实下单始终由用户在外部交易软件完成。
- **安全优先**：平台不保存券商或期货交易凭据，不向真实账户发送委托。
...
5. 将在线推理接入实时买卖点提醒、Paper 模拟、信号跟踪和复盘。
6. 完成外部手动交易辅助、用户成交记录和 AI 决策归因，不接入真实下单通道。
