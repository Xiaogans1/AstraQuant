...
| D7 | 样本外信号失效：10 只板块 ETF proba 均值 0.06–0.14、≥0.5 占比 0%、≤0.35 占比 98–100%——阈值失配 + 训练/部署分布漂移 | 批量报表（`.astraquant/research/reports/sector-etf-batch-report.md`） | Task 6 |
| D8 | 数据深度实测：分钟线回溯 2025 无效（EMPTY）、count=33000 无效——**分钟历史锁死最近约 51 交易日，只能每日增量积累**；**日线 5000 根实测成功（2005-08→2026-08，21 年全开放）** | 东财 SDK 实测（2026-08-08） | Task 9 |
...
### Task 9 — 双轨数据策略：日线全量 + 分钟增量积累（2026-08-08 实测新增）

**Files:** `tools/research/fetch_daily.py`（新增：日线全量拉取）、`tools/research/fetch_minutes.py`（扩展：每日增量）、`packages/data/`（数据集注册）、`research_features.py`（日线特征）

**背景（数据深度实测 D8）**：
- 分钟线：历史锁死最近约 51 交易日（回溯 2025 实测 EMPTY、count=33000 实测 EMPTY、12000 可靠）→ 分钟只能**每日增量积累**。
- 日线：**21 年全开放**（600000 日线 5000 根实测：2005-08-11→2026-08-07）→ 日线可**立即全量拉取**。
- 商用级路径结论：日线模型（趋势/多因子，可解释、多年多窗口验证）是当前最快逼近商用级的轨道；分钟模型（做T/日内）靠积累线并行。

- [ ] **Step 9.1 轨道一：日线全量拉取**
  板块 ETF 全集 + 用户持仓标的，`history_n` 1d 拉取全历史（实测 5000 根 OK），publish 为不可变快照（复用 ParquetSnapshotStore），建立日线数据集注册表（含板块标签、费用属性：ETF 免印花税）。
- [ ] **Step 9.2 日线研究线**
  日线特征（多周期动量/反转/波动率/量能，point-in-time + 防泄漏）+ 日线标签（未来 N 日收益方向/期望收益）→ 复用 Task 6 协议（walk-forward 多年多窗口 + 板块跑分矩阵）。日线信号与分钟信号（做T）为独立信号源，进入策略层后可组合。
- [ ] **Step 9.3 轨道二：分钟每日增量管线**
  `fetch_minutes.py` 支持增量模式：每日收盘后自动拉取当日 240 根（或多日回补），追加到标的分钟数据集（快照模型需支持分段/合并发布：多段快照合并为统一训练集，重复区间去重，与现有 `load_dataset_bars` 兼容）。
- [ ] **Step 9.4 标的扩充**
  分钟/日线标的从 14 只扩充至板块 ETF 全集（30–50 只），统一数据集注册表（板块、费用属性、数据深度元数据）。
- [ ] **Step 9.5 自动化**
  每日增量拉取入本地任务（复用 TaskSupervisor/定时任务），数据质量检查（evaluate_bars 已有）后自动发布。

**完成定义（Done）：** 日线全历史可用并进入研究线；分钟每日增量自动积累；标的扩充完成；注册表含数据深度元数据。
...
- [x] **双轨数据策略（Task 9）**：日线全量立即可拉（21 年实测），分钟每日增量积累（历史锁死 ~51 交易日实测）——日线研究线为商用级最快路径，分钟积累线并行
