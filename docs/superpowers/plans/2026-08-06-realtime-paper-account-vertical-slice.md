...
在现有东方财富只读真实行情上完成首个可运行的量化业务闭环：系统首次进入时自动建立
默认本地模拟账户，用户可录入初始持仓，系统用最新真实行情持续盯市，可提交虚拟买卖指令、
生成虚拟成交、更新现金与持仓、计算实时盈亏，并在应用重启后完整恢复。
...
1. 首次进入自动创建 10 万元默认模拟账户，重复进入直接恢复且不重复创建。
