...
- [ ] 先用 fake backend 测完整 CLI，不需要网络/GPU/权重；验证 row coverage、顺序、原子写入、缺权重、OOM、NaN、部分输出和 upstream import 失败均不发布半份 response。
...
- [ ] 多路径输出聚合为 terminal-return 中位数、10/50/90 分位、上涨路径占比、路径波动；`expected_return` 只表示终点中位收益，不命名为 probability。
