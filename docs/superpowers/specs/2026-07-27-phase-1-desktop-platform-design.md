...
本阶段为后续数据、研究、回测、Paper 模拟和实时决策辅助提供稳定的平台边界，不实现
任何真实行情、策略、模拟资金或买卖提示功能。
...
Windows 开发环境必须让 Tauri 直接持有最终解释器进程。uv 创建的
`.venv\Scripts\python.exe` 是重定向启动器，会产生与服务不同的 PID；桌面端因此从
`pyvenv.cfg` 解析基础解释器，并通过受控 `PYTHONPATH` 加载该工作区环境。这样既保留
虚拟环境依赖，也能严格校验并监管就绪消息中的实际服务 PID。其他平台可直接使用
虚拟环境解释器。
...
- 安全语义令牌：实时数据、信号有效期、Paper、风险、买、卖、警告和错误。
...
- AstraQuant 不接触真实交易凭据；Phase 1 也不接入数据源密钥。
