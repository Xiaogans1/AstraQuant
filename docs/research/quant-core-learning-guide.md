...
> 更新时间：2026-08-07
> 本文档描述的是截至 2026-08-07 **已实现并运行的旧版 demo**（秒级 ML 基线里程碑），不是 v3 正式架构。

> [!WARNING]
> 本文已冻结为 `LEGACY_SEMANTICS / LEGACY_UNVERIFIED` 历史学习资料。文中的 AUC、收益、阈值、“批准”状态和回放结论只记录旧 demo 行为，**不得作为 v3 alpha、模型发布、Paper 晋级或 LIVE 证据**。v3 的唯一权威基线见 [量化核心开源架构设计](../superpowers/specs/2026-08-10-quant-core-open-source-architecture-design.md)。

---

## 1. 一句话总结

旧版 demo 量化核心 = **真实分钟级行情 → 旧版特征 → LightGBM 模型（经旧注册表门槛标记）→ LEAN 式策略分层（信号→目标仓位→风控→执行）→ 简化本地模拟撮合**。规则策略作为回退路径，且曾在当时数据上验证为亏损，改为只出建议不出手。

旧 demo 模型：**LightGBM 二分类**（预测未来 5 分钟上涨概率），特征为 10 个分钟级微观结构特征；下文只复述当时声称的 **Purged/Embargo 时间切分**与**含费用收益**口径，不代表已满足 v3 的时态、lockbox、真实费率或发布契约。
...
在线特征（1 分钟 K 线 → 10 特征）    ← AlphaModel 输入
   ↓
AlphaModel（旧注册表标记 APPROVED 的 LightGBM / 规则回退）
   ↓
...
- **研究工具链**（`tools/research/`）：
  ...
  - `publish_model.py`：旧版门槛校验 + legacy 注册状态
